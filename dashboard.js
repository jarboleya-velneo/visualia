...
(function () {
  ...
  function render(){
    ...
    chartIVG(s,chartPairs); donut(catTotals(pairs)); chartNet(s,chartPairs); budget(pairs); conceptos(pairs); clients(pairs); fillMovFilters(); movTable();
  }
  ...
  function budget(pairs){
    ...
    var rows=[];
    mNames("categoriasIngreso").forEach(function(k){ var r=inc[k]||0,bud=budForPairs(k,pairs); if(r||bud)rows.push({k:k,real:r,bud:bud,good:true}); });
    mNames("categoriasGasto").forEach(function(k){ var r=real[k]||0,bud=budForPairs(k,pairs); if(r||bud)rows.push({k:k,real:r,bud:bud,good:false}); });
  ...
  function conceptos(pairs){
    ...
    if(!keys.length){ $("#conBody").innerHTML='<tr><td colspan="5" style="color:var(--ink-3)">Sin gastos en el periodo.</td></tr>'; $("#chartCon").innerHTML=""; return; }
    $("#conHint").textContent="Cuánto cuesta cada cosa · mensual equivalente y total · "+periodLabel();
    var tot=0; keys.forEach(function(k){tot+=by[k].total;});
    $("#conBody").innerHTML=keys.map(function(k){ var o=by[k];
      var natTag=o.nat==="directo"?'<span class="tag dir">Directo'+(o.cliente?" · "+esc(o.cliente):"")+'</span>':'<span class="tag ind">Indirecto</span>';
      return '<tr><td><b>'+esc(k)+'</b></td><td>'+esc(o.proveedor||"—")+'</td><td>'+natTag+'</td><td class="r">'+eur2(o.mes)+'</td><td class="r"><b>'+eur2(o.total)+'</b></td></tr>';
    }).join("")+'<tr style="border-top:2px solid var(--line-strong)"><td><b>Total</b></td><td></td><td></td><td></td><td class="r"><b>'+eur2(tot)+'</b></td></tr>';
    // barras horizontales (top 10)
    var top=keys.slice(0,10),W=460,rowH=26,padL=4,padR=60,H=top.length*rowH+6;
    var max=Math.max.apply(null,top.map(function(k){return by[k].total;}));
    var g="";
    top.forEach(function(k,i){ var o=by[k],w=Math.max(2,(W-padL-padR)*o.total/max),y=i*rowH+4;
      g+='<rect x="'+padL+'" y="'+y+'" width="'+w+'" height="16" rx="4" fill="'+(o.nat==="directo"?"var(--s4)":"var(--s1)")+'" data-tip="<b>'+esc(k)+'</b><br><b>'+eur2(o.total)+'</b> en el periodo · '+eur2(o.mes)+'/mes"/>';
      g+='<text x="'+(padL+6)+'" y="'+(y+12)+'" style="font-size:10.5px;fill:#fff;font-weight:650">'+esc(k.length>34?k.slice(0,33)+"…":k)+'</text>';
      g+='<text class="axis-num" x="'+(padL+w+6)+'" y="'+(y+12)+'">'+eur(o.total)+'</text>'; });
    $("#chartCon").innerHTML='<svg viewBox="0 0 '+W+' '+H+'" role="img" aria-label="Gastos por concepto">'+g+'</svg>'+
      '<div class="legend" style="margin-top:8px"><span><i class="dot" style="background:var(--s4)"></i> Directo</span><span><i class="dot" style="background:var(--s1)"></i> Indirecto</span></div>';
    wireTips("#chartCon");
  }
  ...
  function movTable(){
    ...
    if(rows.length){
      var tImp=0,tAn=0;
      var now=new Date();
      rows.forEach(function(it){ tImp+=(it.pct>0?ingresosMes(now.getFullYear(),now.getMonth())*it.pct/100:it.importe);
        tAn+=(it.freq==="recurrente"?arr12(it):(it.pct>0?arr12(it)/12:it.importe)); });
      $("#movBody").innerHTML+='<tr style="border-top:2px solid var(--line-strong);cursor:default"><td></td><td><b>Total · '+rows.length+' líneas</b></td><td></td><td></td><td></td><td class="r"><b>'+eur2(tImp)+'</b></td><td class="r"><b>'+eur2(tAn)+'</b></td><td></td></tr>';
    }
